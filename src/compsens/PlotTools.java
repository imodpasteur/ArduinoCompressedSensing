...
/**
 * This plugin demonstrates how to use ImageJ's Plot class to create log-log,
 * semi-log and vector plots, and plots with multiple curves with error bars.
 * It requires ImageJ 1.47r or later.
 *
 * @author Philippe Carl
*/
public class PlotTools implements PlugIn {
    public void plotBasis(double y[], String title) {
	// Plot a basis of the measurement matrix
	if (IJ.versionLessThan("1.47r")) {
	    System.out.println("Error: ImageJ version should be >1.47r");
	    return;
	}

	//double[] x = {113.7700, 88.7468, 56.4684, 30.2856, 3.0181, -27.8690, -58.7503, -84.5998, -118.5439, -114.1901, -91.5908, -59.0922, -32.4929, -3.0302, 27.7297, 60.1355, 86.4298, 113.9279, 116.0125, 116.9311, 115.2122, 113.6640, 114.0277, 113.3385, 111.7978, -113.2085, -113.7728, -112.8976, -116.5197, -115.1823, -114.6198, -116.1608};
	//double[] y = {274.9993, 287.5826, 294.7450, 304.6684, 296.8976, 299.4591, 298.1470, 288.9624, 277.5624, -277.6678, -290.2371, -292.4418, -300.0705, -298.5476, -298.2827, -297.7287, -288.1574, -276.4060, 209.0291, 139.2655, 70.3950, -2.0079, -66.5041, -137.0845, -208.1839, 207.9907, 139.6942, 69.1772, -0.2823, -68.2797, -137.1580, -203.4029};
	...
	// Plot
	Plot plot = new Plot(title, "z position (au)", "Light intensity (ua)");
	plot.setLimits(-1, y.length, 0, 110);
	plot.setSize(600, 600);
	// add red boxes
	plot.setColor(Color.red);
	plot.setLineWidth(2);
	plot.addPoints(x, y, Plot.BOX);
	plot.show();
    }
